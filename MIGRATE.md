...
3. **Install** the plugin — you pick:
   - **A. Marketplace** (versioned, native `claude plugin update`): `claude plugin marketplace add ~/Code/studio-os` → `claude plugin install studio@standard-works`.
...
- `/studio:studio` orients (roles, gates, outputs).
- A gate agent (`cd`) and a workflow (`/studio:design`) load and behave as before — **no behavioral regression**.
